...
// resolveClaude locates the claude binary. Prefers the SSD-bundled
// bin/claude over $PATH so compile stays SSD-autark.
func resolveClaude(myhubRoot string) (string, error) {
	ssdBin := filepath.Join(myhubRoot, "bin", "claude")
	if info, err := os.Stat(ssdBin); err == nil && info.Mode()&0o111 != 0 {
		return ssdBin, nil
	}
	if p, err := exec.LookPath("claude"); err == nil {
		return p, nil
	}
	return "", errors.New("claude binary not found on SSD or $PATH")
}
