...
- [ ] 2.1 Global `:focus-visible` token + universal two-tone ring (`outline: 2px solid var(--focus-ring); outline-offset: 2px;` + `box-shadow: 0 0 0 4px rgba(0,0,0,.4)` for contrast on both themes).
- [ ] 2.2 Global `@media (prefers-reduced-motion: reduce)` guard zeroing animation/transition durations (already partial; make it root-level reset).
- [ ] 2.3 24×24 hit-area pass — audit Tiptap toolbar, window-control buttons, slash menu items, status-bar buttons. Pad rather than scale.
- [ ] 2.4 `scroll-padding-top` on the main scroll container = title-bar height; satisfies WCAG 2.4.11 (Focus Not Obscured) automatically.
- [ ] 2.5 cmdk a11y patch: add `aria-live="polite"` region announcing "N results"; verify `aria-activedescendant` references the visible active option; ensure virtualization keeps the active row mounted.
- [ ] 2.6 Tiptap toolbar a11y wiring: `role="toolbar" aria-label="Formatting"`, roving tabindex, **Alt+F10** to enter / **Esc** to exit (industry convention), `aria-pressed` reflecting active mark at caret, polite live region announcing toggles.
...
- [ ] 2.8 glide-data-grid: provide `ariaLabel` on all custom cells; add polite live region announcing "Row N of M, column X, value Y" on caret moves.
- [ ] 2.9 Toast roles: confirm Sonner uses `role="status"` for routine and `role="alert"` for failures; dismiss button needs `aria-label="Dismiss"`.
...
- [ ] 2.13 Extract duplicated `resolve_claude` from `commands/ai.py` and `commands/brief.py` into `core/bin_resolve.py` (stub already exists).

## Phase 3 — Shell, Tauri Chrome, Navigation Mechanics

- [ ] 3.1 Custom title bar with **Tauri 2.4 native `trafficLightPosition`** in `tauri.conf.json` (~`{x: 18, y: 22}` for a 56px sidebar header). Drop any `cocoa` crate hackery.
- [ ] 3.2 Install `tauri-plugin-window-state`: persist window size/position per-vault (key = `monitor_signature + vault_id`); validate restored bounds against current monitors; fall back to centered if no intersection.
...
- [ ] 3.7 Pane state persistence per workspace: serialize left/right widths to `memory/config.toml`; restore on mount.
- [ ] 3.8 Double-click a sash to reset adjacent panes to a 20/55/25 default (VS Code pattern).
- [ ] 3.9 Add `key` prop on `<AppShell />` to force unmount/remount when `state.status` changes — fixes the Unlock → AppShell flash described in shell audit.
- [ ] 3.10 Persistent "?" help button in StatusBar visible at all breakpoints; opens ShortcutsOverlay. Solves discoverability when shortcuts hint-row is hidden at <900px.
...
- [ ] 3.12 "Local" pill in title bar (Arc-inspired): clickable; expands into plain-English "Your notes never leave this drive" with technical detail behind a "How?" disclosure.
...
- [ ] 4.1 Voice rewrite: in `Onboarding.tsx`, `Unlock.tsx`, `Settings.tsx`, `CommandPalette.tsx`, replace tokens:
      - "Vault" → "Drive lock" / "Passphrase" (context-dependent)
      - "Credentials" → "AI access"
      - "Provider" → "AI engine"
      - "Compile wiki" → "Refresh wiki"
      - "Verify drive" → "Check drive"
      - "Briefer" → "Daily briefing"
      - "Subprocess" → hidden entirely
...
- [ ] 4.3 Caps-Lock detector on Unlock: show inline warning icon + text when `KeyboardEvent.getModifierState('CapsLock')` is true.
...
- [ ] 4.5 Back button on Onboarding steps 2–4 (disabled on step 1). Step indicator "Step N of 4" or progress dots.
...
- [ ] 4.9 Trust callout in Onboarding passphrase step — visual + copy: "Your data stays on this drive. Anthropic sees your prompts, no one else does." Reduces fear.
